...
export default function Labores(){
    ...
    async function load(){ try{ const res = await api.fetchLabores(); setItems(res.data) }catch(err){ console.error(err) } }

    async function save(e){ e.preventDefault(); const desc = e.target.descripcion.value; try{ if(editing){ await api.updateLabor(editing.id || editing.ID, { descripcion: desc }) } else { await api.createLabor({ descripcion: desc }) } setEditing(null); e.target.reset(); load() }catch(err){ alert('Error') } }
    async function remove(id){ if(!confirm('Eliminar?')) return; try{ await api.deleteLabor(id); load() }catch(err){ alert('Error') } }
    function startEdit(it){ setEditing(it) }

    return (
        <div className="grid lg:grid-cols-3 gap-6">
            <div className="card lg:col-span-2">
                <h3 className="text-lg font-semibold">Labores Agronómicas</h3>
